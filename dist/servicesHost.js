...
function makeServicesHost(scriptRegex, loader, instance, enableFileCaching, projectReferences) {
    ...
    files.forEach((_value, key) => {
        if (!utils_1.fileMatchesPatterns(instance.loaderOptions.emitOnly, key)) {
            files.delete(key);
        }
    });
    ...
    const fileExists = (filePathToCheck) => {
        const shouldEmit = utils_1.fileMatchesPatterns(instance.loaderOptions.emitOnly, filePathToCheck);
        if (!shouldEmit) {
            return false;
        }
        if (instance.solutionBuilderHost) {
